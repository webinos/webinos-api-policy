                    ...
                    policyeditor.getPolicySet(0, function(ps) {
                        ...
                        policyeditor.testPolicy(ps, request, function(res) {
                            if ((access == "enable" && res.effect != 0) ||
                                ...
                                var newPs = editPolicy(policyeditor, ps, userId, serviceId, access, request);
                                if (newPs != null) {
...
var editPolicy = function (pe, ps, userId, serviceId, access, request) {
    var policy = ps.getPolicy([userId]);
    if (policy.matched) {
        policy = policy.matched[0].toJSONObject();
    } else if (policy.generic) {
        policy = policy.generic[0].toJSONObject();
    } else {
        return null;
    }

    var removedResourceMatch = false, addedResourceMatch = false;
    ...
    console.log("policy after remove: " + JSON.stringify(ps.toJSONObject()));
    ps.addPolicy(policy, 0);
    console.log("the row after addPolicy");
    console.log("policy after add: " + JSON.stringify(ps.toJSONObject()));
