...
var editPolicy = function (pe, ps, access, request, res) {
    ...
    if (requestorId != null) {
        ...
        if (policySet.matched.length > 0) {
            ...
            if (policySet.target[0].subject[0]['subject-match'][0].$.match === friendsURI || userIds.length > 1) {
                if (userIds.length > 1) {
                    ...
                    policSet.target[0].subject[0]['subject-match'][0].$.match = userIds.toString();
                }
                ...
                for (var i = 0; i < policySet.policy.length; i++) {
                    ...
                    policySet.policy[i].$.id = 'p_' + requestorId + '_' + date;
                    policySet.policy[i].$.description = requestorId + '-policy';
                    for (var j = 0; j < policySet.policy[i].rule.length; j++) {
                        policySet.policy[i].rule[j].$.id = 'r_' + requestorId + '_' + ++date;
                    }
                }
            }
        ...
        if (policy != null) {
            ...
            if (requestorIds.length > 1) {
                ...
                // modify policy's and rules' ids
                policy.$.id = 'p_' + requestorId + '_' + date;
                policy.$.description = requestorId + '-policy';
                for (var i = 0; i < policy.rule.length; i++) {
                    policy.rule[i].$.id = 'r_' + requestorId + '_' + ++date;
                }
                // modify target to replace the generic URI
        ...
        // add new policy
            ...
            policy = createNewPolicy(ps, 'requesto-id', requestorId, date);
            policySet.policy.splice(0, 0, policy);
        }
        policy = addResource(policy, requestorId, serviceId, access, ++date);
    ...
    // policy without devices
        ...
        if (policy.matched.length > 0) {
            ...
            // new user, add policy
            policy = createNewPolicy(ps, 'user-id', userId, date);
        }
...
var createNewPolicy = function (ps, attr, match, date) {
    var policy = ps.createPolicy('p_' + match + '_' + date, 'first-applicable', match + '-policy');
    var subject = {};
