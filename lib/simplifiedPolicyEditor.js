...
var editPolicy = function (pe, ps, access, request, res) {
    ...
    if (requestorId != null) {
        ...
        if (policySet.matched.length > 0) {
            ...
            // policy set with friends URI
            if (policySet.target[0].subject[0]['subject-match'][0].$.match === friendsURI) {
                // TODO manage policy set with friends uri
            }
        ...
        // add new policySet
            ...
            policySet = ps.createPolicySet('ps_' + userId + '_' + new Date().getTime(), 'first-matching-target',userId + '-policy');
            policySet = policySet.toJSONObject();
            // add default rule
            policySet.policy = [];
            policySet.policy[0].rule = [];
            policySet.policy[0].rule.push({'$' : {'effect' : 'deny', 'id' : 'r_' + userId + '_default'}});
        }
        if (policy != null) {
        ...
        // add new policy
            ...
            policy = createNewPolicy(ps, 'requesto-id', requestorId);
            policySet.policy.splice(0, 0, policy);
        }
        policy = addResource(policy, requestorId, serviceId, access);

        var newPolicySet = new pe.policyset(policySet);
        ps.addPolicySet(newPolicySet, position);
    ...
    // policy without devices
        ...
        if (policy.matched.length > 0) {
            ...
            // new user, add policy
            policy = createNewPolicy(ps, 'user-id', userId);
        }

        policy = addResource(policy, userId, serviceId, access);
...
var createNewPolicy = function (ps, attr, match) {
    var policy = ps.createPolicy('p_' + match + '_' + new Date().getTime(), 'first-applicable', match + '-policy');
    var subject = {};
    subject['subject-match'] = [];
    subject['subject-match'].push({'$' : {'attr' : attr, 'match' : match}});
    policy.addSubject('s_' + match, subject);
    policy = policy.toJSONObject();
    // add default rule
    policy.rule = [];
    policy.rule.push({'$' : {'effect' : 'deny', 'id' : 'r_' + match + '_default'}});
    return policy;
}

var addResource = function (policy, Id, serviceId, access) {
    ...
    if (addedResourceMatch == false) {
        ...
        rule.$.id = 'r_' + Id + '_' + new Date().getTime();
        rule.condition = [];
        rule.condition[0] = {};
        rule.condition[0].$ = {};
