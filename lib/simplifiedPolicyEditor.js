...
var editPolicy = function (pe, ps, access, request, res) {
    ...
    // policy with devices
    if (requestorId != null) {
        var fullPolicy = ps.toJSONObject();
        ...
        if (policySet.matched.length > 0) {
            ...
            // workaround to find policySetPosition
            for (var i = 0; i < fullPolicy['policy-set'].length; i ++) {
                if (fullPolicy['policy-set'][i].$.id === policySet.$.id) {
                    policySetPosition = i;
                    break;
                }
            }
            ...
            if (policy != null) {
                // workaround to find policyPosition
                for (var i = 0; i < policySet.policy.length; i ++) {
                    if (policySet.policy[i].$.id === policy.$.id) {
                        policyPosition = i;
                        break;
                    }
                }
